...
-- This function is used to create a snippet that automatically adds a backslash before the trigger
...
-- Auto symbol
-- This function is used to create a snippet that expands symbols to a command with a backslash
...
-- Text format
M.text_format_snippet = function(context, command, opts)
	opts = opts or {}
	if not context.trig then
		error("context doesn't include a `trig` key which is mandatory", 2)
	end
	...
	return autosnippet({
		trig = context.trig,
		trigEngine = "pattern",
		-- dscr = "Expands " .. context.trig .. " to LaTeX's " .. command .. "{} command.",
		name = context.name,
		docstring = context.docstring,
	}, fmta(command .. "{<>}", { d(1, M.get_visual) }), opts)
end
